...
(function(){
  ...
  self.addEventListener('sync', function(event) {
    // test-tag-from-devtools is for emulated sync call.
    // fetchStocks is what we have registered in the app.
    if (event.tag == 'fetchStocks' || event.tag == 'test-tag-from-devtools') {
      console.log('fetchStocks bg fired!');
      self.clients.matchAll().then(all => {
        all.map(client => client.postMessage('Online'))
      });
    };
  });
